...
simple_wflow %>%
  fit() %>%
  # `.x` is the already defined recipe in the workflow
  update_recipe(~ .x %>% step_mutate(cyl = log10(cyl)))
...
If you specify a recipe, you can either update it or provide a completely new recipe, if you update a new dataset, it replaces the old one, if you update a new model it replaces the old one. Now, let's think how this workflow works with more challenging examples.
...
# Update the data and fit everything
simple_wflow %>%
  update_data(mtcars2) %>% 
  fit()
...
simple_wflow %>%
  update_recipe(~ .x %>% step_add(am)) %>% 
  fit()

# Update the recipe to include one more term and add a step
simple_wflow %>%
  update_recipe(~ .x %>% step_add(am) %>% step_dummy(disp)) %>% 
  fit()
...
Another scenario is one in which the user wants to remove and update the complete formula. `update_recipe` will accept two things: either an anonymous function, which interprets to be an update, or a new recipe, which will replace the previous one. For example:
...
simple_wflow %>%
  update_recipe(
    # This is an idea in-progress because `.x` has no binding value.
    # Perhaps I can capture the unevaluated expression and replace
    # .x with the data but specifying this makes it look like breaking
    # the rules. One idea is to use the dummy `rlang::.data` as a place
    # holder.
    recipe(mpg ~ cyl + gear, data = .x) %>%
      step_center(cyl) %>%
...
simple_wflow <-
  mtcars %>%
  tflow() %>%
  add_split(initial_split, prop = 0.75) %>%
  add_recipe(~ {
    recipe(mpg ~ cyl + gear + disp, data = .x) %>%
      step_mutate(cyl = log10(cyl)) %>%
      step_dummy(gear) %>%
