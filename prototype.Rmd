...
simple_wflow %>%
  fit() %>%
  # complete_recipe will pass the already existing recipe to
...
simple_wflow <-
  ...
  add_model(set_engine(linear_reg(), "lm")) %>%
  # New_step
  complete_recipe(~ .x %>% step_mutate(cyl = log10(cyl)))
